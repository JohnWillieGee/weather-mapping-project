...
# ═══════════════════════════════════════════════════════════════════════════════
#
# Normalised outage record shape (all distributors):
#   {
#     "distributor": str,   # "ausgrid" | "endeavour" | "energex" | "ergon"
#     "state":       str,   # "NSW" | "QLD"
#     "type":        str,   # "unplanned" | "planned"
#     "customers":   int,   # affected customer count (0 if unknown)
#     "suburb":      str,   # suburb / location name
#     "cause":       str,   # cause description if available
#     "est_restore": str,   # estimated restoration time (ISO string or human readable)
#     "lat":         float, # centroid latitude  (None if unavailable)
#     "lng":         float, # centroid longitude (None if unavailable)
#     "polygon":     list,  # [[lng,lat],...] boundary coords if available (else None)
#   }
#
# All feeds are fetched server-side (no CORS issues).
# Ausgrid SOAP bounding box covers greater Sydney, Central Coast, Hunter region.
# ═══════════════════════════════════════════════════════════════════════════════

AUSGRID_SOAP_URL = "https://www.ausgrid.com.au/services/Outage/Outage.asmx"

AUSGRID_SOAP_ENVELOPE = """<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
               xmlns:xsd="http://www.w3.org/2001/XMLSchema"
               xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <GetOutages xmlns="http://www.ausgrid.com.au/services/outage/">
      <bottomLeftLat>{blat}</bottomLeftLat>
      <bottomLeftLng>{blng}</bottomLeftLng>
      <topRightLat>{tlat}</topRightLat>
      <topRightLng>{tlng}</topRightLng>
      <zoomLevel>8</zoomLevel>
      <isFuture>false</isFuture>
    </GetOutages>
  </soap:Body>
</soap:Envelope>"""

# Bounding box: covers Ausgrid service territory (Sydney, Central Coast, Hunter, ACT fringe)
AUSGRID_BBOX = {"blat": -35.0, "blng": 149.0, "tlat": -32.0, "tlng": 152.5}
...
ENERGEX_PLANNED_URL    = "https://www.energex.com.au/static/Energex/energex_po_current_planned.geojson"
ERGON_UNPLANNED_URL    = "https://www.ergon.com.au/static/Ergon/ergon_po_current_unplanned.geojson"
...
def _centroid(coords):
    """Compute rough centroid of a polygon ring [[lng,lat],...]."""
    if not coords:
        return None, None
    ...
    if not lats:
        return None, None
    return round(sum(lats)/len(lats), 5), round(sum(lngs)/len(lngs), 5)


def _geo_to_poly_and_centroid(geo_shape):
    """
    Extract polygon coords + centroid from a GeoJSON geometry dict.
    Returns (lat, lng, polygon) where polygon is [[lng,lat],...] of outer ring.
    """
    if not geo_shape:
        return None, None, None
    gtype  = geo_shape.get("type", "")
    coords = geo_shape.get("coordinates", [])
    try:
        if gtype == "Point":
            return round(coords[1], 5), round(coords[0], 5), None
        elif gtype == "Polygon":
            ...
            biggest = max(coords, key=lambda p: len(p[0])) if coords else []
...
def fetch_ausgrid_outages():
    """
    Fetch current outages from Ausgrid SOAP webservice.
    Returns (list_of_outages, error_string_or_None)
    """
    envelope = AUSGRID_SOAP_ENVELOPE.format(**AUSGRID_BBOX)
    try:
        r = requests.post(
            AUSGRID_SOAP_URL,
        ...
        r.raise_for_status()
        print(f"  Ausgrid SOAP: {len(r.content)} bytes")
    except requests.RequestException as e:
    ...
    try:
        ...
        ns = {"s": "http://schemas.xmlsoap.org/soap/envelope/",
              "o": "http://www.ausgrid.com.au/services/outage/"}
        outages = []
        # Walk all elements looking for outage records (structure varies by response)
        for item in root.iter():
            tag = item.tag.split("}")[-1] if "}" in item.tag else item.tag
            if tag.lower() in ("outage", "outageitem", "item"):
                lat_el  = item.find(".//{*}Latitude") or item.find(".//{*}lat")
                lng_el  = item.find(".//{*}Longitude") or item.find(".//{*}lng")
                cust_el = item.find(".//{*}CustomersAffected") or item.find(".//{*}customers")
                sub_el  = item.find(".//{*}Suburb") or item.find(".//{*}suburb")
                est_el  = item.find(".//{*}EstimatedRestorationTime") or item.find(".//{*}ert")
                cause_el = item.find(".//{*}Cause") or item.find(".//{*}cause")
                try:
                    ...
                    customers = int(cust_el.text) if cust_el is not None and cust_el.text else 0
                except (ValueError, TypeError):
                    lat, lng, customers = None, None, 0
                outages.append({
                    "distributor": "ausgrid",
                    "state":       "NSW",
                    "type":        "unplanned",
                    "customers":   customers,
                    "suburb":      sub_el.text.strip() if sub_el is not None and sub_el.text else "",
                    "cause":       cause_el.text.strip() if cause_el is not None and cause_el.text else "",
                    "est_restore": est_el.text.strip() if est_el is not None and est_el.text else "",
                    "lat":         lat,
                    "lng":         lng,
                    "polygon":     None,
                })
        print(f"  Ausgrid: {len(outages)} outages parsed")
        return outages, None
    except ET.ParseError as e:
        return [], f"XML parse error: {e}"


def fetch_endeavour_outages():
    """
    Fetch current outages from Endeavour Energy open data REST API (GeoJSON).
    Returns (list_of_outages, error_string_or_None)
    """
    outages = []
    errors  = []

    for label, url, otype in [
        ("unplanned", ENDEAVOUR_UNPLANNED_URL, "unplanned"),
        ("planned",   ENDEAVOUR_PLANNED_URL,   "planned"),
    ]:
        try:
            ...
            print(f"  Endeavour {label}: {len(records)} records")
            for rec in records:
                geo   = rec.get("geo_shape", {}) or {}
                lat, lng, poly = _geo_to_poly_and_centroid(geo)
                try:
                    customers = int(rec.get("customers_affected") or 0)
                except (ValueError, TypeError):
                    customers = 0
                est = (rec.get("estimated_restoration_time") or
                       rec.get("estimated_end_time") or "")
                outages.append({
                    "distributor": "endeavour",
                    "state":       "NSW",
                    "type":        otype,
                    "customers":   customers,
                    "suburb":      str(rec.get("suburb") or ""),
            ...
            errors.append(f"endeavour_{label}: {e}")
        except (ValueError, KeyError) as e:
            errors.append(f"endeavour_{label}_parse: {e}")

    err = "; ".join(errors) if errors else None
    return outages, err


def _parse_energex_ergon_geojson(data, distributor, state, otype):
    """Parse Energex/Ergon GeoJSON FeatureCollection into normalised outage records."""
    outages = []
    for feat in data.get("features", []):
        ...
        lat, lng, poly = _geo_to_poly_and_centroid(geo)
        try:
            customers = int(props.get("customersAffected") or
                           props.get("customers_affected") or
                           props.get("Customers") or 0)
        except (ValueError, TypeError):
            customers = 0
        suburb = (props.get("suburb") or props.get("Suburb") or
                  props.get("location") or props.get("Location") or "")
        cause  = (props.get("cause") or props.get("Cause") or
                  props.get("outageType") or "")
        est    = (props.get("estimatedRestorationTime") or
                  props.get("estimated_restoration_time") or
                  props.get("restorationTime") or "")
        outages.append({
...
def fetch_energex_ergon_outages():
    """
    Fetch current outages from Energex and Ergon direct GeoJSON feeds.
    Returns (list_of_outages, error_string_or_None)
    """
    feeds = [
    ...
    for url, dist, state, otype in feeds:
        try:
            ...
            print(f"  {dist.capitalize()} {otype}: {len(batch)} outages")
            outages.extend(batch)
        except requests.RequestException as e:
            errors.append(f"{dist}_{otype}: {e}")
        except (ValueError, KeyError) as e:
            errors.append(f"{dist}_{otype}_parse: {e}")
...
def fetch_all_outages():
    """
    Fetch outages from all configured distributors.
    Returns (outages_dict, error_dict)
      outages_dict = { "nsw": [...], "qld": [...] }
    """
    all_outages = []
    errors = {}

    print("Fetching Ausgrid outages (SOAP)...")
    ag_list, ag_err = fetch_ausgrid_outages()
    all_outages.extend(ag_list)
    if ag_err:
        errors["ausgrid"] = ag_err

    print("Fetching Endeavour Energy outages (REST)...")
    en_list, en_err = fetch_endeavour_outages()
    all_outages.extend(en_list)
    if en_err:
        errors["endeavour"] = en_err

    print("Fetching Energex/Ergon outages (GeoJSON)...")
    eq_list, eq_err = fetch_energex_ergon_outages()
    all_outages.extend(eq_list)
    if eq_err:
        errors["energex_ergon"] = eq_err

    # Group by state for easier consumption by the page
    by_state = {
        "nsw": [o for o in all_outages if o["state"] == "NSW"],
        "qld": [o for o in all_outages if o["state"] == "QLD"],
    }
